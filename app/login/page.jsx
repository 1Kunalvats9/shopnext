    ...
    try {
      ...
      if (res.ok) {
        try {
          ...
          console.error('Login succeeded but failed to parse JSON:', responseText, parseError);
    ...
    <div className='bg-white w-[100vw] h-[100vh] flex items-center gap-5 justify-center'>
      <form className="max-w-96 w-full text-center border px-8 bg-gray-400/10 backdrop-blur-md border-white/30 rounded-xl shadow-sm p-6" onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={loading}
          className={`mt-2 w-full h-11 rounded-full text-white bg-indigo-500 transition-opacity cursor-pointer ${loading ? 'opacity-60 cursor-not-allowed' : 'hover:opacity-90'}`}
        >
